<?php
    ...
    {
        mysqli_query($db,"DELETE FROM elections WHERE id = '".$_GET['deleteid']."'") OR die(mysqli_error($db));
        ?>
                ...
                while($row = mysqli_fetch_assoc($fetchingData))
                {
                    ...
                    ?>
                            <tr>
                               ...
                               <td>
                                <a href="ondx.php?editData=<?php echo $elctionid; ?>" class="btn btn-sm btn-warning"> Edit </a>
                                <button class="btn btn-sm btn-danger" onclick="DeleteData('<?php echo $elctionid; ?>')"> Delete</button>
                               </td>
...
<script>
    const DeleteData = (e_id) =>
    {
        ...
        if(c == true)
        {
           location.assign("ondx.php?addElectionPage=1&deleteid=" + e_id);
        }
    }
</script>
...
<?php
    ...
    if(isset($_POST['addElectionBtn']))
    {
        ...
        // inserting into db
        mysqli_query($db,"INSERT INTO elections(electiontopic, noofcandidates, startingdate, endingdate, status, insertedby, insertedon)VALUES('". $election_topic."','".$number_of_candidates."',
        '".$starting_date."','".$ending_date."','".$status."','".$inserted_by."','".$inserted_on."')") or 
        die(mysqli_error($db));

        ?>
    <script> location.assign("ondx.php?addElectionPage=1&added=1")</script>
